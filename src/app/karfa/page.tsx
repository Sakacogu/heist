...
export default function CartPage() {
  ...
  const handleCheckout = () => {
    ...
    clearCart();
    setModal('Stripe greiðslusíða er væntanleg 😊\nPöntunin þín var vistuð á prófílnum.');
  };
    ...
    <div className="max-w-6xl mx-auto md:flex md:gap-10 p-16">
        ...
        {items.map((i) => (
          <div key={i.cartId}
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => updateQty(i.cartId, Math.max(1, i.qty - 1))}
                      className="px-3 py-1 bg-gray-100 rounded">−</button>
              <span className="min-w-[2ch] text-center">{i.qty}</span>
              <button onClick={() => updateQty(i.cartId, i.qty + 1)}
                      className="px-3 py-1 bg-gray-100 rounded">+</button>
            </div>
            ...
            <button onClick={() => removeItem(i.cartId)}
                    className="text-red-500 text-lg ml-4" title="Remove item">
              ✕
            </button>
          </div>
        ))}
      ...
      <aside className="md:w-80 mt-10 md:mt-0">
        <div className="bg-white p-6 rounded-xl shadow space-y-4 sticky top-24">
          ...
          {nextTier && (
            <div className="mb-2 bg-cyan-50 p-3 rounded-lg text-sm text-cyan-800
                            flex justify-between items-center">
              <span>
                {nextTier.min - itemQty} vör{nextTier.min - itemQty === 1 ? 'u' : 'ur'} til viðbótar →
          ...
          <input type="text" placeholder="Afsláttarkóði"
